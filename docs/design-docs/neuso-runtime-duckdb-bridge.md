...
English TL;DR: DuckDB should call NeuSO during join-order optimization with a large regular inner join graph plus optimizer-stage estimates, receive a linear relation-id join order, validate it, and apply it as a forced left-deep plan with native fallback.
...
如果模型不可用、超时、返回非法结果，DuckDB 必须回退到原生 join-order optimizer。NeuSO 接入不应该让一条本可执行的查询失败。
...
SQL
  ...
  -> NeuSO sidecar 或 native runtime 推理
...
关键点是：NeuSO 输出不直接替换 SQL，也不直接生成 DuckDB `LogicalOperator`。DuckDB 应该把 NeuSO 返回的 `join_order` 转成 join-order optimizer 能理解的 forced plan，再复用 `QueryGraphManager::Reconstruct()` 生成 logical join tree。
...
DuckDB 收到非 `ok` status 后应该回退原生 optimizer。
...
如果任一校验失败，或 sidecar 超时、崩溃、返回 malformed JSON，DuckDB 必须 fail closed：

```text
记录调试 metadata
丢弃 NeuSO response
继续使用 DuckDB 原生 PlanEnumerator / approximate greedy path
```

这种回退策略是运行时接入的安全边界。NeuSO 只能改善可支持子图的 join-order decision，不能降低 DuckDB 对普通 SQL 的可靠性。
...
- `input.sql`：给开发人员阅读的 SQL 场景，说明这个 case 对应哪条 join query。当前 regression 模式不执行它；未来 `duckdb-export` 模式可以复用它驱动 DuckDB 生成真实 export。
- `input_request.json`：当前实际喂给 NeuSO adapter 的输入，模拟 DuckDB 在线优化阶段传出的 runtime request。
- `expected_response.json`：标准答案，只包含稳定字段，例如 `status`、`model_version`、`join_order`。`latency_ms` 这类非确定字段不参与 regression 精确比较。

初始 `chain_12` case 表示 12 张表按 `t0.i = t1.i = ... = t11.i` 形成的 regular inner chain join。request 包含 12 个 relation、11 条 inner edge，以及基础 cardinality、estimated cardinality、pair cardinality、selectivity 和可选 estimated join cost。期望 response 是：
...
实际文件中还保留 `version`、`request_id` 和 `graph_hash`，用于确认 response 对应的是同一个 request。
...
`actual_response.json` 保留 `latency_ms`，`actual_response.normalized.json` 是去掉非确定字段后参与 regression 对比的内容。如果 actual 和 expected 不一致，runner 会打印 unified diff 并以非零状态退出。
...
- normalized response 与 `expected_response.json` 精确一致。
